  ...
  async ({ target }, ctx) => {
    ...
    try {
      ...
      if (target === "production") {
        const { imported, skipped, studyLinks, soloStudiesCreated } = await importProductionLibrary()
        output =
          ...
          `${soloStudiesCreated} paper(s) with no Study yet got a solo one so they show up in browse.`
      } else {
